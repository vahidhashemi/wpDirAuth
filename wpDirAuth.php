 ...
 * wpDirAuth version.
 */
define('WPDIRAUTH_VERSION', '1.7.17');
...
//        echo "I am here";
    ...
    function wpDirAuth_auth($username, $password)
    {
        ...
        elseif ( ! $isLoggedIn) {
            ...
            return wpDirAuth_retrieveUserDetails($username,$connection,$baseDn,$filterQuery,$results);
...
//                exit;
    ...
    function wp_authenticate($username, $password)
    {
        ...
        elseif ($enable) {
             ...
             * Directory auth == true
             */
//            echo "ooooh dirAuth is enabled"; exit;
...
//                        if ($userID = wp_create_user($userLogin, $password, $userEmail)) {
                        $aryUserDetails['user_pass'] = $password;//we're going to store a random password in WP since directory users will never use it to log in anyway'
                        $aryUserDetails['user_login'] = $username;
                        $aryUserDetails['user_email'] = $aryUserDetails['email'] = $username;

                        $tmpAr1 = explode('@',$username);
                        $aryUserDetails['nickname'] =  str_replace('.','_',$tmpAr1[0]);
                        $aryUserDetails['display_name'] = $username;
                        $aryUserDetails['role'] = "author";
//                        $intUserID = wp_insert_user($aryUserDetails);

                        if ($userID = wp_insert_user($aryUserDetails)) {
                            $userData['ID'] = $userID;
     ...
     */
    function wpDirAuth_retrieveUserDetails($username,$rscConnection,$strBaseDn,$strFilterQuery,$rscResult=false){
